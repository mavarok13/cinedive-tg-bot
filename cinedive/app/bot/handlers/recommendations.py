...
from cinedive.app.services.recommendation_service import DiscoveryStrategy, RecommendationService
from cinedive.app.services.tmdb_service import TMDBService

router = Router(name="recommendations")
logger = logging.getLogger(__name__)

RECOMMENDATION_BATCH_SIZE = 40
MIN_CANDIDATES_BEFORE_SECOND_DISCOVERY = 15
MAX_DISCOVER_PAGES_PER_REFILL = 6
MAX_DISCOVER_PAGES_SECOND_PASS = 3
MAX_DETAIL_FETCHES_PER_REFILL = 60
MAX_DETAIL_FETCHES_SECOND_PASS = 20
RECENTLY_SHOWN_COOLDOWN_DAYS = 30
TMDB_MAX_DISCOVER_PAGE = 500
MAX_EMPTY_RESULTS_PER_STRATEGY = 3
PENALTY_TTL_DAYS = 45
...
async def _send_recommendation(
    ...
    queue_item = await queue_repo.claim_next_unshown(
        user_id=user_id,
...
async def _build_recommendation_queue(
    *,
    ...
    media_repo = MediaRepository(session)
    queue_repo = RecommendationQueueRepository(session)
    locked_mood_session = await queue_repo.lock_mood_session(mood_session_id=mood_session.id)
    if locked_mood_session is None:
        return
    if await queue_repo.has_unshown(user_id=user_id, mood_session_id=mood_session.id, now=now):
        return
    ...
    strategies = recommendation_service.build_discovery_strategies(
        content_type=mood_session.content_type,
        favorite_genre_ids=favorite_genre_ids,
        mood_genre_ids=mood_genre_ids,
    )
    logger.info(
        "Starting recommendation refill: user_id=%s mood_session_id=%s strategies=%s",
        user_id,
        mood_session.id,
        len(strategies),
    )

    excluded_media_ids = await queue_repo.media_ids_for_session(mood_session_id=mood_session.id)
    await _seed_discover_candidates(
        ...
        now=now,
        max_pages=MAX_DISCOVER_PAGES_PER_REFILL,
        max_detail_fetches=MAX_DETAIL_FETCHES_PER_REFILL,
    )

    candidates = await _load_recommendation_candidates(
        ...
        mood_session=mood_session,
        now=now,
        shown_cooldown_days=RECENTLY_SHOWN_COOLDOWN_DAYS,
    )
    if len(candidates) < MIN_CANDIDATES_BEFORE_SECOND_DISCOVERY:
        excluded_media_ids = await queue_repo.media_ids_for_session(mood_session_id=mood_session.id)
        await _seed_discover_candidates(
            session=session,
            user_id=user_id,
            ...
            excluded_media_ids=excluded_media_ids,
            now=now,
            max_pages=MAX_DISCOVER_PAGES_SECOND_PASS,
            max_detail_fetches=MAX_DETAIL_FETCHES_SECOND_PASS,
        )
        candidates = await _load_recommendation_candidates(
            media_repo=media_repo,
            queue_repo=queue_repo,
    ...
    if not candidates:
        candidates = await _load_recommendation_candidates(
            ...
            mood_session=mood_session,
            now=now,
            shown_cooldown_days=0,
        )

    logger.info(
        "Recommendation refill candidate pool: user_id=%s mood_session_id=%s candidates=%s",
        user_id,
        mood_session.id,
        len(candidates),
    )
    ...
    queue_items = recommendation_service.build_queue(
        candidates,
        favorite_genre_ids=favorite_genre_ids,
        mood_genre_ids=mood_genre_ids,
    ...
    if not queue_items:
        await session.commit()
        return
    await queue_repo.enqueue_batch(
        user_id=user_id,
        mood_session_id=mood_session.id,
    ...
    logger.info(
        "Recommendation refill queued items: user_id=%s mood_session_id=%s queued=%s",
        user_id,
        mood_session.id,
        len(queue_items),
    )
...
async def _load_recommendation_candidates(
    *,
    media_repo: MediaRepository,
    queue_repo: RecommendationQueueRepository,
    user_id: int,
    language_code: str,
    mood_session: UserMoodSession,
    now: datetime,
    ...
    excluded_media_ids = await queue_repo.media_ids_for_session(mood_session_id=mood_session.id)
    return await media_repo.list_recommendation_candidates(
        user_id=user_id,
        ...
        now=now,
        limit=300,
        shown_cooldown_days=shown_cooldown_days,
...
async def _seed_discover_candidates(
    *,
    ...
    try:
        ...
        logger.exception(
            "TMDB discovery unavailable: user_id=%s mood_session_id=%s",
            user_id,
            mood_session.id,
        )
        return
    ...
    try:
        state_repo = RecommendationDiscoveryStateRepository(session)
        states = await state_repo.ensure_states(
            user_id=user_id,
            mood_session_id=mood_session.id,
            strategies=[strategy.draft for strategy in strategies],
        )
        strategy_by_key = {
            (strategy.media_type, strategy.sort_by, strategy.genre_key, strategy.filter_key): strategy
            for strategy in strategies
        }
        media_repo = MediaRepository(session)
        genre_names_cache: dict[str, dict[int, str]] = {}
        for state in states:
            if pages_attempted >= max_pages or detail_fetches >= max_detail_fetches:
                break
            if state.next_page > TMDB_MAX_DISCOVER_PAGE:
                ...
                continue
            strategy = strategy_by_key.get((state.media_type, state.sort_by, state.genre_key, state.filter_key))
            if strategy is None:
                continue
            ...
            try:
                results = await tmdb.discover_media(
                    ...
                    genre_ids=strategy.genre_ids,
                    max_runtime_minutes=mood_session.max_runtime_minutes,
                    sort_by=strategy.sort_by,
                    page=state.next_page,
                    vote_count_gte=strategy.vote_count_gte,
                    vote_average_gte=strategy.vote_average_gte,
                    original_language=strategy.original_language,
                    origin_country=strategy.origin_country,
                )
            except (httpx.HTTPError, RuntimeError, ValueError):
                logger.exception(
                    "TMDB discover failed: user_id=%s mood_session_id=%s media_type=%s sort_by=%s genre_key=%s filter_key=%s page=%s",
                    user_id,
                    mood_session.id,
                    state.media_type,
                    state.sort_by,
            ...
            usable_raw_count = 0
            raw_ids = {result.get("id") for result in results if isinstance(result.get("id"), int)}
            existing_ids = await media_repo.existing_external_ids(
                source="tmdb",
                media_type=state.media_type,
                external_ids={int(tmdb_id) for tmdb_id in raw_ids},
            )
            for result in results:
                tmdb_id = result.get("id")
                if not isinstance(tmdb_id, int):
                    continue
                if existing_ids.get(tmdb_id) in excluded_media_ids:
                    continue
                if existing_ids.get(tmdb_id) is not None:
                    usable_raw_count += 1
                    continue
                if not result.get("poster_path") or not result.get("overview"):
                    continue
                if detail_fetches >= max_detail_fetches:
                    break
                usable_raw_count += 1
                try:
                    details = await _fetch_details(tmdb, tmdb_id, state.media_type, _tmdb_language(locale))
                    await _persist_media_details(
                        session,
                        tmdb,
                        details,
                        state.media_type,
                    ...
                    logger.exception(
                        "TMDB detail persistence failed: user_id=%s mood_session_id=%s media_type=%s tmdb_id=%s",
                        user_id,
                        mood_session.id,
                        state.media_type,
                        tmdb_id,
                ...
                detail_fetches += 1
            empty = usable_raw_count == 0
            next_empty_count = state.empty_result_count + 1 if empty else 0
            await state_repo.record_attempt(
                state=state,
                next_page=state.next_page + 1,
                empty=empty,
                exhausted=next_empty_count >= MAX_EMPTY_RESULTS_PER_STRATEGY
                or state.next_page >= TMDB_MAX_DISCOVER_PAGE,
                used_at=now,
            )
    finally:
        if tmdb is not None:
            await tmdb.aclose()
    logger.info(
        "TMDB discovery refill finished: user_id=%s mood_session_id=%s pages=%s raw=%s details=%s persisted=%s",
        user_id,
        mood_session.id,
        pages_attempted,
        raw_count,
...
async def _persist_media_details(
    ...
    media_type: str,
    locale: str,
    genre_names_cache: dict[str, dict[int, str]] | None = None,
) -> None:
    ...
    if genre_names_cache is None:
        genre_names = await _canonical_genre_names(tmdb, media_type)
    else:
        if media_type not in genre_names_cache:
            genre_names_cache[media_type] = await _canonical_genre_names(tmdb, media_type)
        genre_names = genre_names_cache[media_type]
    genre_ids: list[int] = []
